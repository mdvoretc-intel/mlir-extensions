...
#include "imex/Dialect/gml_st/transforms/fusion.h"
#include "imex/Dialect/gml_st/transforms/linalg_utils.h"
#include "imex/Dialect/gml_st/transforms/passes.h"
#include "imex/Dialect/gml_st/transforms/tiling.h"
#include "imex/Dialect/gml_st/transforms/tiling_interface_impl.h"
#include "imex/Dialect/gml_st/transforms/transforms.h"
...
struct TilePartialSoftmaxPattern
  ...
  TilePartialSoftmaxPattern(MLIRContext *ctx, bool distribute,
      ...
      : OpInterfaceRewritePattern<TilingInterface>(ctx, benefit),
        distribute(distribute), tileSizes(std::move(tileSizes)) {}
  ...
  LogicalResult matchAndRewrite(TilingInterface op,
    ...
    bool hasFusableOccurrences =
        llvm::any_of(op->getUsers(), [](Operation *op) {
          return llvm::isa<MaterializeOp>(op);
        });
...
struct FusePartialSoftmaxPattern : public OpRewritePattern<MaterializeOp> {
  ...
  LogicalResult matchAndRewrite(MaterializeOp op,
    ...
    if (!llvm::isa<TilingInterface>(def))
      return failure();
...
struct FuseUnaryCwisePattern : public OpRewritePattern<MaterializeOp> {
  ...
  LogicalResult matchAndRewrite(MaterializeOp op,
    ...
    if (!isUnaryCwiseGenericOp(source))
      return failure();
