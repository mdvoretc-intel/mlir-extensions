...
#include "imex/Dialect/gml_st/transforms/rewriters.h"
...
struct BufferizeExtractSliceOp : public OpConversionPattern<ExtractSliceOp> {
  ...
  LogicalResult
  matchAndRewrite(ExtractSliceOp op, OpAdaptor adaptor,
...
struct BufferizeEmptyTensorOp : public OpConversionPattern<EmptyOp> {
  ...
  LogicalResult
  matchAndRewrite(EmptyOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    if (!op->getParentOfType<LoopOp>())
      return failure();
...
Value findExistingSubview(Value destMemRef) {
  if (auto toMemref = destMemRef.getDefiningOp<ToMemrefOp>()) {
    if (auto toTensor = toMemref.getTensor().getDefiningOp<ToTensorOp>()) {
      if (!isBlockArgOfTiledLoop(toTensor.getMemref()))
        return Value{};
        ...
        if (auto anotherCast = mlir::dyn_cast<ToMemrefOp>(tensorUser)) {
          for (Operation *memrefUser : anotherCast.getMemref().getUsers()) {
            if (auto subview = mlir::dyn_cast<SubViewOp>(memrefUser)) {
              if (subview.getSource() == destMemRef)
                return subview;
            }
          }
        }
...
public:
  ...
  LogicalResult
  matchAndRewrite(InsertSliceOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
ValueRange getVariadicOperands(DenseI32ArrayAttr sizeAttr, ValueRange operands,
                               unsigned index) {
  const int32_t *sizeIt = &*sizeAttr.value_begin<int32_t>();
  if (sizeAttr.isSplat())
    return operands.slice(*sizeIt * index, *sizeIt);
...
struct BufferizeLinalgYieldOp : public OpConversionPattern<gml_st::YieldOp> {
  ...
  LogicalResult
  matchAndRewrite(gml_st::YieldOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
private:
  LogicalResult runDialectConversionBasedBufferization() {
    ...
    target.addLegalDialect<
        ...
        mlir::complex::ComplexDialect, mlir::AffineDialect,
