...
/// include/imex/Dialect/gml_st/transforms/passes.td.
struct GmlStToGpuPass : public ::impl::GmlStToGpuPassBase<GmlStToGpuPass> {
  void runOnOperation() override {
    ...
    target.addDynamicallyLegalOp<linalg::GenericOp>([](linalg::GenericOp op) {
      return llvm::none_of(op.getIteratorTypes().getAsValueRange<StringAttr>(),
                           [](StringRef type) {
...
static LogicalResult getInductionVarIdxForLevel(unsigned nestingLevel,
  ...
  if (nestingLevel >= kNestingToLaunchIdx.size())
    return failure();
...
static Value handleImperfectTile(Location loc, LaunchOp launch,
  ...
  if (!affineMin || affineMin.getMap().getNumResults() != 2)
    return upperBound;
  auto tileSize =
...
// we encounter.
LogicalResult
ParallelOpToGpuPattern::matchAndRewrite(ParallelOp root,
...
mlir::gml_st::createGmlStToGpuPass() {
  return std::make_unique<GmlStToGpuPass>();
}
