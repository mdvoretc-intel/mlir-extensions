...
struct TransferReadOfOneDimExpandShape
  ...
  mlir::LogicalResult
  matchAndRewrite(TransferReadOp vectorRead,
...
struct VectorizeGmlStLoopsPass
  ...
  void runOnOperation() override {
    ...
    auto isValidDistribution = [&](Operation *op) {
      if (distributionLabels.empty())
        return true;
      ParallelOp parent = op->getParentOfType<ParallelOp>();
      if (!parent || !parent.getDistributionType().has_value())
        return false;
...
createVectorizeGmlStLoopsPass(bool vectorizeGmlStOps,
