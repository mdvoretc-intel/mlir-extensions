...
ParseResult
parseShapeTypeDimensionsList(AsmParser &parser,
...
#include "imex/Dialect/gml_st/IR/gml_st_dialect.cc.inc"

// Generated type classes.
#define GET_TYPEDEF_CLASSES
#include "imex/Dialect/gml_st/IR/gml_st_types.cc.inc"

// Generated attribute classes.
#define GET_ATTRDEF_CLASSES
#include "imex/Dialect/gml_st/IR/gml_st_attrs.cc.inc"
...
void GmlStDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "imex/Dialect/gml_st/IR/gml_st_ops.cc.inc"
      >();
  addTypes<
...
ParseResult LoopOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (succeeded(parser.parseOptionalKeyword("iterators"))) {
    if (parser.parseAttribute(iterTypes))
      return failure();
  } else {
  ...
  if (succeeded(parser.parseOptionalKeyword("distribution"))) {
    ...
    if (failed(parser.parseAttribute(distributionTypes)))
      return failure();
...
struct LoopInputsFolder : public OpRewritePattern<LoopOp> {
  ...
  LogicalResult matchAndRewrite(LoopOp loop,
    ...
    if (newInputs.size() == loop.getInputs().size())
      return failure();
...
struct LoopResultsFolder : public OpRewritePattern<LoopOp> {
  ...
  LogicalResult matchAndRewrite(LoopOp loop,
    ...
    if (newOutputOperands.size() == loop.getOutputs().size())
      return failure();
...
struct TensorCastOfLoopInsOutsFolder : public OpRewritePattern<LoopOp> {
  ...
  LogicalResult matchAndRewrite(LoopOp loop,
    ...
    if (!inputCasts.castFound && !outputCasts.castFound)
      return failure();
...
private:
  ...
  SmallVector<Value, 4>
  insertCastsAndCloneBody(const CastOpsOfArgs &inputCasts,
                          const CastOpsOfArgs &outputCasts, LoopOp loop,
                          LoopOp newLoop, PatternRewriter &rewriter) const {
