...
#include "imex/Dialect/kgen/Transforms/tiling.h"
...
OpFoldResult computeTileSizeInDim(OpBuilder &builder, Location loc,
  ...
  if (hasTileSizeOne || dividesEvenly)
    return builder.getIndexAttr(*tileCst);
  ...
  LogicalResult matchAndRewrite(tensor::DimOp op,
    ...
    if (auto materializeOp = llvm::dyn_cast<MaterializeOp>(def)) {
      ...
      if (!set.getType().isa<TileType>())
        return failure();
      rewriter.replaceOpWithNewOp<kgen::SizeOp>(op, set, op.getIndex());
      return success();
    }
    return failure();
  }
...
/// `kgen.for` for iterating over the tiles.
struct TilingPattern : public OpInterfaceRewritePattern<TilingInterface> {
  TilingPattern(MLIRContext *context,
      ...
      : OpInterfaceRewritePattern<TilingInterface>(context, benefit),
        filterFn(filterFn), options(std::move(options)) {}
...
struct TilingPass : public impl::TilingPassBase<TilingPass> {
  ...
  void getDependentDialects(DialectRegistry &registry) const final {
    registry
        .insert<kgen_Dialect, tensor::TensorDialect, linalg::LinalgDialect>();
    registerKgenTilingInterfaceExternalModels(registry);
  }
  ...
  void runOnOperation() override {
    ...
    auto filterFn = [&](Operation *op) {
      if (!opName.empty() && op->getName().getStringRef() != opName)
        return failure();
      if (!opLabel.empty() && !hasMatchingLabel(op, opLabel))
        return failure();
      return success();
    };
...
FailureOr<TilingResult> tile(const TilingOptions &options,
  ...
  // Implement adding accumulator to the kgen.parallel terminator.
  ...
  // 5. Add `kgen.set_yield` terminator.
  ...
  if (!options.distribute) {
    auto forLoop = cast<kgen::ForOp>(tilingResult.loop);
...
createTilingPass(StringRef opName, StringRef opLabel, bool distribute,
                 ArrayRef<int64_t> tileSizes) {
