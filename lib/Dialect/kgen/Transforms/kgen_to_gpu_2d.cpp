...
#define GEN_PASS_DEF_kgenTOGPU2DPASS
#include "imex/Dialect/kgen/Transforms/Passes.h.inc"

using namespace mlir;
using namespace mlir::kgen;
using mlir::gpu::LaunchOp;

namespace {
/// Converts a sequence of 2 nested kgen.parallel ops into a gpu.launch op.
...
///
/// Each kgen.parallel is expected to have up to 3 induction variables, to be
/// mapped onto the X, Y and Z dimensions respectively.
///
/// All operations from within the nested kgen.parallel regions are copied
...
/// Implements the kgenToGpu2dPass declared in
/// include/imex/Dialect/kgen/transforms/passes.td.
struct kgenToGpu2dPass : public ::impl::kgenToGpu2dPassBase<kgenToGpu2dPass> {
  void runOnOperation() override {
    RewritePatternSet patterns(&getContext());
    patterns.add<ParallelOpToGpu2dPattern>(&getContext());
    ConversionTarget target(getContext());
    target.addIllegalDialect<kgen_Dialect>();
    ...
    // We're producing new ops (clones of original ops in kgen.parallel
...
static Value matchLaunchSpaceToLoop(ParallelOp parallel,
  ...
  // Compute the value that kgen.parallel's induction variable would have in
...
// Converts the 2 nested kgen.parallel ops rooted at `root` into a
// gpu.launch op. We do this by creating an empty gpu.launch region and
// copying all the operations in kgen.parallel into that region,
// recursively copying the bodies of any nested kgen.parallel regions that
// we encounter.
LogicalResult
